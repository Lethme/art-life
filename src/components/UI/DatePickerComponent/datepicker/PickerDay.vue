<template>
  ...
  >
    ...
    <section
      v-if="ifDifferentViews && selectedDate"
    ...
    >
      ...
      <p class="vuejs3-datepicker__calendar-topbar-day">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function isPreviousMonthDisabled(): boolean {
      ...
      return (
        getMonth(d.to) >= getMonth(t) && getFullYear(d.to) >= getFullYear(t)
      );
    }
    ...
    function isNextMonthDisabled(): boolean {
      ...
      const t = props.pageDate;
      return (
        getMonth(d.from) <= getMonth(t) && getFullYear(d.from) <= getFullYear(t)
      );
    }
    ...
    function isDisabledDate(date: number | Date): boolean {
      ...
      if (typeof t.dates !== "undefined") {
        t.dates.forEach(
          (d: { getTime: () => string | number | Date }): void => {
      ...
      if (
        typeof t.days !== "undefined" &&
        t.days.indexOf(getDay(date as Date)) !== -1
      ) {
        disabledDates = true;
      }
      if (
        typeof t.daysOfMonth !== "undefined" &&
        t.daysOfMonth.indexOf(getDate(date as Date)) !== -1
      ) {
        disabledDates = true;
      }
    ...
    function isHighlightedDate(date: number | Date): boolean {
      ...
      if (
        typeof h.days !== "undefined" &&
        h.days.indexOf(getDay(date as Date)) !== -1
      ) {
        highlighted = true;
      }

      if (
        typeof h.daysOfMonth !== "undefined" &&
        h.daysOfMonth.indexOf(getDate(date as Date)) !== -1
      ) {
        highlighted = true;
      }
    ...
    function dayClasses(day: {
      ...
      return {
        ...
        "selected-first": day.isSelected && (props.selectedDates[1] || props.highlightedDates?.to) && datesEqual(new Date(day.timestamp), props.selectedDates[0]) && !datesEqual(new Date(day.timestamp), this.highlightedDates?.to),
    ...
    const daysOfWeek = computed(() => {
      if (props.mondayFirst) {
        const tempDays =
          props.translation &&
          props.translation.days &&
          props.translation.days.slice();
    ...
    const isYmd = computed(() => {
      return (
        (props.translation &&
          props.translation.ymd &&
          props.translation &&
          props.translation.ymd) === true
      );
    });
    ...
    const getDayName = computed(() => {
      ...
      return props.selectedDate
        ? getDayNameAbbr(
            propDate,
            props.translation && props.translation.daysNames
          )
        : null;
    });
    ...
    const ifDifferentViews = computed(() => {
      return !(
        props.minimumView === props.maximumView &&
        (props.minimumView !== "day" || props.maximumView !== "day")
      );
    });
